...
- Build the three-legged Linux router (`zn-gw`) that joins the segments and later enforces for the PLC.
- Build the two-tier application: `zn-app01` (nginx) and `zn-db01` (PostgreSQL).
- Build the Windows SCADA/HMI workstation (`zn-win01`).
- Build the agentless "PLC" (`zn-ot01`) that answers Modbus TCP and can host no agent.
...
### Lab 4.1 — Build `zn-gw`, the three-legged router

**Objective.** Create the Ubuntu router with one NIC on each network, forwarding and masquerading.

**Walkthrough**

**Step 1.** New VM: Ubuntu Server 22.04.5, 1 vCPU, 1024 MB, 20 GB. Add **three** NICs: NIC1 → **VMnet8**, NIC2 → **VMnet2**, NIC3 → **VMnet3**. Hostname `zn-gw`, user `labadmin`, OpenSSH enabled.

**Step 2.** Identify interfaces (`ip -br link`, typically `ens33/ens34/ens35`) and write the netplan:

```bash
sudo tee /etc/netplan/00-zn-gw.yaml > /dev/null <<'EOF'
network:
  version: 2
  ethernets:
    ens33:
...
EOF
sudo chmod 600 /etc/netplan/00-zn-gw.yaml
sudo netplan apply
```
...
**Negative test.** Skip `net.ipv4.ip_forward=1` and internal hosts reach `zn-gw` but not the internet.

**Rollback.** None — read-only; this lab changes no persistent state, so there is nothing to revert.

### Lab 4.2 — Build `zn-db01`, the PostgreSQL tier

**Objective.** Create the database, the crown jewel this lab protects.

**Walkthrough**

**Step 1.** New VM: Ubuntu Server 22.04.5, 1536 MB, one NIC → **VMnet2**. Hostname `zn-db01`, `10.10.20.12/24`, gateway `10.10.20.254`, DNS `192.168.170.2`.

**Step 2.** Install PostgreSQL and create the app database:

```bash
sudo apt update && sudo apt -y install postgresql
sudo -u postgres psql <<'SQL'
CREATE DATABASE znlab;
CREATE USER appuser WITH PASSWORD 'LabAppPassw0rd!';
GRANT ALL PRIVILEGES ON DATABASE znlab TO appuser;
\c znlab
...
echo "host znlab appuser 10.10.20.0/24 md5" | \
...
### Lab 4.3 — Build `zn-app01`, the nginx application tier

**Objective.** Create the web/app tier whose only legitimate east-west dependency is the database.

**Walkthrough**

**Step 1.** New VM: Ubuntu Server 22.04.5, 1536 MB, one NIC → **VMnet2**. Hostname `zn-app01`, `10.10.20.11/24`, gateway `10.10.20.254`, DNS `192.168.170.2`.
...
echo "zn-app01 nginx up" | sudo tee /var/www/html/index.html
cat > ~/checkdb.sh <<'EOF'
#!/usr/bin/env bash
PGPASSWORD='LabAppPassw0rd!' psql -h 10.10.20.12 -U appuser -d znlab \
...
### Lab 4.4 — Build `zn-win01`, the Windows SCADA/HMI workstation
...
Rename-Computer -NewName "zn-win01" -Restart
```
...
### Lab 4.5 — Build `zn-ot01`, the agentless PLC

**Objective.** Create the unpatchable device that answers Modbus TCP 502 and can host no agent.

**Walkthrough**

**Step 1.** New VM: Ubuntu Server 22.04.5, 768 MB, 10 GB, one NIC → **VMnet3**. Hostname `zn-ot01`, `10.10.30.50/24`, gateway `10.10.30.254`, DNS `192.168.170.2`.
...
**Expected result.** The listener is bound on `10.10.30.50:502`; from `zn-gw`, `nc -vz 10.10.30.50 502` connects.
...
- [ ] `zn-gw` routes and masquerades; internal hosts reach the internet.
- [ ] `zn-db01` serves PostgreSQL on `10.10.20.12:5432` with the `znlab` database.
- [ ] `zn-app01` serves nginx and reaches the database (`~/checkdb.sh` prints 3).
- [ ] `zn-win01` is up as the SCADA/HMI workstation.
- [ ] `zn-ot01` answers Modbus TCP 502 and is reachable only via `zn-gw`.
