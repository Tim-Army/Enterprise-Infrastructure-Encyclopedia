...
- Dell Technologies, *iDRAC9/iDRAC10 User's Guide* — Power, Thermal, Logs,
  and Alerts chapters
- Dell Technologies, *iDRAC RACADM CLI Guide* — `iDRAC.Power`,
  `iDRAC.ThermalSettings`, `getsel`, `lclog`, and `techsupreport` command
  reference
- Dell Technologies, *iDRAC Redfish API Guide* — `Chassis`, `LogServices`,
