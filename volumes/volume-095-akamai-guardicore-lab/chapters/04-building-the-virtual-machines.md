...
- Build the three-legged Linux router (`gc-gw`) that joins the segments and later enforces for the PLC.
- Build the two-tier application: `gc-app01` (nginx) and `gc-db01` (PostgreSQL).
- Build the Windows SCADA/HMI workstation (`gc-win01`).
- Build the agentless "PLC" (`gc-ot01`) that answers Modbus TCP and can host no agent.
- Snapshot every VM at a known-good baseline.

Every static address comes from the Chapter 01 address plan. Keep it in front of you.

## Hands-On Lab

### Lab 4.1 — Build `gc-gw`, the three-legged router
...
**Step 1.** New VM: Ubuntu Server 22.04.5, 1 vCPU, 1024 MB, 20 GB. Add **three** NICs: NIC1 → **VMnet8**, NIC2 → **VMnet2**, NIC3 → **VMnet3**. Hostname `gc-gw`, user `labadmin`, OpenSSH enabled.

**Step 2.** Identify the interfaces (`ip -br link`, typically `ens33/ens34/ens35`) and write the netplan:

```bash
sudo tee /etc/netplan/00-gc-gw.yaml > /dev/null <<'EOF'
network:
  version: 2
  ethernets:
    ens33:
...
EOF
sudo chmod 600 /etc/netplan/00-gc-gw.yaml
sudo netplan apply
```

**Step 3.** Enable forwarding and masquerade the internal segments:
...
**Step 1.** New VM: Ubuntu Server 22.04.5, 1536 MB, one NIC → **VMnet2**. Hostname `gc-db01`, address `10.10.20.12/24`, gateway `10.10.20.254`, DNS `192.168.170.2`.
...
CREATE DATABASE gclab;
CREATE USER appuser WITH PASSWORD 'LabAppPassw0rd!';
GRANT ALL PRIVILEGES ON DATABASE gclab TO appuser;
\c gclab
...
echo "host gclab appuser 10.10.20.0/24 md5" | \
...
**Step 1.** New VM: Ubuntu Server 22.04.5, 1536 MB, one NIC → **VMnet2**. Hostname `gc-app01`, `10.10.20.11/24`, gateway `10.10.20.254`, DNS `192.168.170.2`.
...
echo "gc-app01 nginx up" | sudo tee /var/www/html/index.html
cat > ~/checkdb.sh <<'EOF'
#!/usr/bin/env bash
PGPASSWORD='LabAppPassw0rd!' psql -h 10.10.20.12 -U appuser -d gclab \
...
Rename-Computer -NewName "gc-win01" -Restart
```
...
**Expected result.** Both succeed now — the flat network permits everything. That is the problem this lab fixes.
...
**Step 1.** New VM: Ubuntu Server 22.04.5, 768 MB, 10 GB, one NIC → **VMnet3**. Hostname `gc-ot01`, `10.10.30.50/24`, gateway `10.10.30.254`, DNS `192.168.170.2`.
...
**Expected result.** The listener is bound on `10.10.30.50:502`; from `gc-gw`, `nc -vz 10.10.30.50 502` connects.
...
Shut down all five guests cleanly, then take a snapshot named `baseline` on each (**VM → Snapshot → Take Snapshot**).
...
- [ ] `gc-gw` routes and masquerades; internal hosts reach the internet.
- [ ] `gc-db01` serves PostgreSQL on `10.10.20.12:5432` with the `gclab` database.
- [ ] `gc-app01` serves nginx and reaches the database (`~/checkdb.sh` prints 3).
- [ ] `gc-win01` is up as the SCADA/HMI workstation.
- [ ] `gc-ot01` answers Modbus TCP 502 and is reachable only via `gc-gw`.
