...
- Write a change record complete enough to satisfy the four
  safe-administration gates from Chapter 01 (authorization, backup, dry
  run, rollback plan).
...
and vendor hardening guides (Chapter 07) are examples of externally
...
gates introduced in Chapter 01, now formalized as a document rather than
...
- **Store templates and baselines as code, in the same repository
  discipline as application source** (Volume I, Chapter 02): version
...
- **Right-size change record formality to risk tier** (Chapter 01's
...
Every Tier 1–3 change (Chapter 01) should produce a record containing, at
...
- **When drift is detected, classify it before reacting.** Determine
  ...
  Chapter 07), an undocumented emergency change (reconcile into the
...
- Require pull request review for any change to a baseline or template
  ...
  pattern established in Volume I.
...
- Periodically re-validate that the current baseline still matches the
  current CIS Benchmark or vendor hardening guide version (Chapter 07);
...
- Volume I, Chapter 02 — Repository Architecture (version control
  discipline applied to configuration).
- Volume I, Chapter 03 — Automation Architecture (plan/apply separation).
- Volume IX — Infrastructure Automation (full Ansible/Terraform template
...
- CIS Benchmarks (`cisecurity.org/cis-benchmarks`) — external baseline
  source referenced in Chapter 07.
- Chapter 01 of this volume — the four safe-administration gates that
...
- [ ] I wrote a complete change record satisfying all four
      safe-administration gates from Chapter 01.
