...
**Objective.** Provide an isolated segment on `10.10.30.0/24` with **no host adapter** and **no DHCP**, so the only path to it is through `tf-gw`.
...
**Expected result.** No output. The host cannot reach `10.10.30.0/24` directly — the property that makes `tf-gw` the single, complete enforcement point for the PLC.

**Negative test.** Ticking the host adapter here gives the host a path that bypasses `tf-gw`; every enforcement result in Chapter 08 would then be measured on the wrong path. Leave it unchecked.
...
**Step 2.** The OT cell is reachable from the host only through `tf-gw`. You will not add a host route to it — testing the PLC in Chapter 08 is done from `tf-gw` or a Data Center host, which legitimately transit the enforcement point.
