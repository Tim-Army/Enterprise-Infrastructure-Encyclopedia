...
- **Automation authentication model.** A shared, long-lived API key used
  ...
  retrieved from a secrets manager at runtime ([Volume IX](../../volume-009-infrastructure-automation/README.md)), and rotation on
  a defined schedule.
...
- **Terraform vs. Ansible for PAN-OS automation.** Favor the Terraform
  ...
  ([Volume VII](../../volume-007-cloud-infrastructure/README.md), [Volume IX](../../volume-009-infrastructure-automation/README.md)) and state-tracked drift detection is valuable.
  Favor the Ansible Collection where the organization's existing
  operational automation is already Ansible-centric ([Volume IX](../../volume-009-infrastructure-automation/README.md)) or where a
...
- [Volume IX](../../volume-009-infrastructure-automation/README.md) — Infrastructure Automation, for plan/apply and idempotency
...
This chapter carries a topic-level walkthrough lab for the **Network
Security Analyst Domain 3 (Management and Operations) and Domain 4
(Troubleshooting)** and the NGFW Engineer **Domain 3 (Integration and
Automation)** — upgrades, packet capture, session troubleshooting,
configuration management, log forwarding, and the XML/REST APIs — mapped in
the volume README's coverage table. Each is a full walkthrough and ends
**`**Lab verified by:** *pending*`** until a human runs it.

**Shared prerequisites for Labs 7.1–7.7** — a licensed PAN-OS 11.x firewall
(HA pair for Lab 7.1), CLI as `admin`, and API access from a host with
`curl`. The firewall reachable at `$FW`. **Cost:** none beyond lab resources.

### Lab 7.1 — Perform an HA-aware PAN-OS upgrade (Domain 3: Operations)

**Objective:** Upgrade content and software on an HA pair without downtime.

```text
admin@pa-fw01> request content upgrade check
admin@pa-fw01> request content upgrade download latest
admin@pa-fw01> request content upgrade install version latest
admin@pa-fw01> request system software download version 11.1.4
admin@pa-fw01> request system software install version 11.1.4
```

**Expected result:** content installs on both members; software installs on
the passive member first, which is then made active before upgrading the
former active — sessions stay up throughout.

**Negative test:** upgrade both HA members simultaneously; sessions drop
...
**Objective:** Take a filtered dataplane packet capture.

```text
admin@pa-fw01> debug dataplane packet-diag set filter match source 10.10.20.5 destination 8.8.8.8
admin@pa-fw01> debug dataplane packet-diag set filter on
admin@pa-fw01> debug dataplane packet-diag set capture stage receive file rx.pcap
admin@pa-fw01> debug dataplane packet-diag set capture on
admin@pa-fw01> view-pcap filter-pcap rx.pcap
```

**Expected result:** captured packets for the filtered flow at the `receive`
stage — the ground truth for a connectivity fault.
...
**Objective:** Generate an API key and query system state.

```bash
KEY=$(curl -sk "https://$FW/api/?type=keygen&user=admin&password=$PW" | sed -n 's:.*<key>\(.*\)</key>.*:\1:p')
curl -sk "https://$FW/api/?type=op&cmd=<show><system><info></info></system></show>&key=$KEY" | grep -oE '<sw-version>[^<]+'
```

**Expected result:** the firewall's software version returned via the XML
API — the interface automation and monitoring tools build on.
...
**Objective:** Create an address object through the REST API.

```bash
curl -sk -X POST "https://$FW/restapi/v11.1/Objects/Addresses?name=api-host&location=vsys&vsys=vsys1" \
  -H "X-PAN-KEY: $KEY" -H 'Content-Type: application/json' \
  -d '{"entry":{"@name":"api-host","ip-netmask":"10.10.20.99/32"}}'
curl -sk "https://$FW/restapi/v11.1/Objects/Addresses?name=api-host&location=vsys&vsys=vsys1" -H "X-PAN-KEY: $KEY" | grep api-host
```

**Expected result:** the `api-host` object is created and read back — the
REST API drives config as code (Ansible/Terraform build on it).

**Negative test:** POST without committing; the object is in the candidate
...
**Objective:** Forward threat logs to a syslog/Panorama collector.

```text
admin@pa-fw01# set shared log-settings profiles Corp-LFP match-list threat send-syslog SyslogSrv
admin@pa-fw01# set shared server-profile syslog SyslogSrv server LogHost server 10.10.20.20 transport UDP port 514
admin@pa-fw01# set rulebase security rules Allow-Web log-setting Corp-LFP
admin@pa-fw01# commit
admin@pa-fw01> show log threat direction equal backward
```

**Expected result:** threat logs generated by the rule are forwarded to the
syslog server — centralized logging for the SOC.
...
**Objective:** Read an active session's disposition to localize a fault.

```text
admin@pa-fw01> show session all filter source 10.10.20.5 destination 8.8.8.8
admin@pa-fw01> show session id <id>
admin@pa-fw01> show counter global filter delta yes severity drop
```

**Expected result:** the session's state, the rule it matched, and any global
drop counters — the session table plus drop counters name where traffic dies.

**Negative test:** assuming a "no traffic" symptom is a policy deny when
...
**Objective:** Audit and safely roll back a configuration change.

```text
admin@pa-fw01# set rulebase security rules Test-Rule from any to any application any action allow
admin@pa-fw01> configure
admin@pa-fw01# show config diff
admin@pa-fw01# exit
admin@pa-fw01> revert config
admin@pa-fw01> show config saved
```

**Expected result:** `show config diff` shows the pending `Test-Rule` add;
`revert config` discards it, restoring the running config — a safe rollback
of an uncommitted change.

**Negative test:** an `any/any/allow` rule committed without review is a
