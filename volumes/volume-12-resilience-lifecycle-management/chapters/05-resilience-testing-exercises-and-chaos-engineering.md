...
**Negative Test:** Introduce a deliberate bug by moving `rollback_fn()` out of the `finally` block and into the normal return path only, then modify the driver to raise an exception partway through the loop (for example, `raise RuntimeError("simulated tooling crash")` at `elapsed == 2`). Rerun and confirm `degraded` remains `True` after the script exits — demonstrating exactly the unprotected rollback failure mode described in this chapter's Knowledge Checks, and why rollback must be structurally guaranteed rather than merely present in the normal-completion path.
