...
![Lab flow for this chapter: a baseline oscap xccdf eval against the CIS profile records a nonzero fail count on a fresh VM; harden-ssh.yml sets PermitRootLogin no via lineinfile and reloads sshd, and a re-scan shows the root-login rule now passing where it previously failed. As a negative test, PermitRootLogin is manually reverted to yes and sshd reloaded; a targeted re-scan catches the regression, showing the same rule failing again, demonstrating why the automation feedback loop must include re-scanning rather than trusting a one-time remediation.](../../../diagrams/volume-004-enterprise-systems-administration/chapter-08-openscap-ansible-remediation-flow.svg)
...
- Explain how host-level compliance automation in this chapter feeds the
  broader security program covered in [Volume X](../../volume-010-enterprise-cybersecurity/README.md).
...
threat detection, and incident response are covered in [Volume X](../../volume-010-enterprise-cybersecurity/README.md); this
...
- **Protect who can change the baseline automation itself.** Baseline
  ...
  CODEOWNERS/branch-protection discipline from [Volume I](../../volume-001-enterprise-engineering-foundations/README.md) so a change to the
