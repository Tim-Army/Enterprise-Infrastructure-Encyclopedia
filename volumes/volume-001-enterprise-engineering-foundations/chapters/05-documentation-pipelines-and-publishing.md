...
![Lab flow for this chapter: an internal-link checker validates a small markdown source tree before Pandoc builds standalone HTML; with valid links the build proceeds, while a deliberately broken link is caught by the checker and the build never runs.](../../../diagrams/volume-001-enterprise-engineering-foundations/chapter-05-documentation-pipeline-link-validation-flow.svg)
