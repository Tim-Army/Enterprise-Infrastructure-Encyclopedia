...
![Flow diagram showing a port reference card validated by a live TCP handshake capture and a bound-address listening-state check, alongside a connection to an unopened port that distinguishes a reset, an ICMP unreachable, and a silent timeout.](../../../diagrams/volume-999-reference-library/chapter-02-port-reference-handshake-flow.svg)
...
- **Segment by traffic direction, not just by port.** North-south traffic
  ...
  more from micro-segmentation ([Volume V](../../volume-005-vmware-virtualization/README.md) NSX, [Volume X](../../volume-010-enterprise-cybersecurity/README.md)) because a single
...
| Kerberos | 88 | TCP/UDP | Client → KDC | Admin/authentication. Core to Active Directory ([Volume IV](../../volume-004-enterprise-systems-administration/README.md)) authentication. |
...
| iSCSI | 3260 | TCP | Initiator → target | Storage. Isolate on a dedicated storage VLAN/subnet ([Volume VI](../../volume-006-enterprise-storage-data-protection/README.md)). |
| Fibre Channel over Ethernet | N/A (Ethertype 0x8906, not IP) | FCoE | N/A | Storage; not IP-routable, relevant to [Volume VI](../../volume-006-enterprise-storage-data-protection/README.md) data-center fabric design. |
| vCenter Server / ESXi management | 443 (HTTPS UI/API), 902 (legacy host management/vMotion heartbeat) | TCP | Client/host → vCenter | Admin. See [Volume V](../../volume-005-vmware-virtualization/README.md) for the full vSphere port matrix. |
...
| kubelet API | 10250 | TCP | API server → node | Admin; anonymous access must be disabled ([Volume VIII](../../volume-008-containers-platform-engineering/README.md), [Volume X](../../volume-010-enterprise-cybersecurity/README.md)). |
...
| BGP | 179 | TCP | Peer ↔ peer (either side may initiate) | Routing protocol; see [Volume II](../../volume-002-network-engineering-foundations/README.md)/III for adjacency design. |
...
| Redfish / IPMI | 443 (Redfish, HTTPS REST), 623 (IPMI, UDP) | TCP/UDP | Client → BMC | Admin. Out-of-band server management ([Volume XXII](../../volume-022-dell-openmanage-enterprise/README.md)/XXIII); IPMI is legacy and less secure than Redfish. |
...
- **Capture and read the handshake, not just the result.** `tcpdump -ni
  <iface> tcp port <port>` (see [Volume XX](../../volume-020-wireshark-packet-analysis/README.md) for full packet-analysis
...
- Never expose RDP, WinRM, SSH, database ports, or Kubernetes/etcd
  ...
  VPN, or Zero Trust broker ([Volume X](../../volume-010-enterprise-cybersecurity/README.md), [Volume XVI](../../volume-016-palo-alto-networks-security/README.md)) for administrative
  access.
...
- [Volume II](../../volume-002-network-engineering-foundations/README.md) — Network Engineering Foundations (transport-layer theory).
- [Volume V](../../volume-005-vmware-virtualization/README.md) — VMware Virtualization (full vSphere/ESXi port matrix).
- [Volume X](../../volume-010-enterprise-cybersecurity/README.md) — Enterprise Cybersecurity (segmentation and Zero Trust
  architecture).
- [Volume XX](../../volume-020-wireshark-packet-analysis/README.md) — Wireshark and Packet Analysis (packet-level validation of
