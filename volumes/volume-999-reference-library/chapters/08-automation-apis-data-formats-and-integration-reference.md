...
![Flow diagram showing an API reference card built from a documented authentication procedure and one successful authenticated call, tested against both an invalid credential returning 401 and an unauthorized resource returning 403, explicitly distinguished from each other.](../../../diagrams/volume-999-reference-library/chapter-08-api-reference-auth-failure-flow.svg)
