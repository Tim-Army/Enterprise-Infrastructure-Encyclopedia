...
![Flow diagram showing a command quick-reference card populated with real Tier 0 output and a Tier 1 change gated through authorization, backup, dry run, and rollback before execution, alongside a mistyped read-only command whose error text becomes a documented common-failure note.](../../../diagrams/volume-999-reference-library/chapter-01-command-card-safe-admin-gates-flow.svg)
