...
![Lab flow for this chapter: lv_lab (1 GB origin) is written with point-in-time A data, then a 100 MB snapshot lv_lab_snap is taken and the origin is modified to point-in-time B; the snapshot mounted read-only still shows point-in-time A while the origin shows point-in-time B, proving independent preservation. As a negative test, enough new data is written to the origin to exceed the snapshot's 100 MB reserve; data_percent reaches 100%, the snapshot's lv_attr shows an invalidated state, and mounting it afterward fails or exposes a partial image — demonstrating why proactive snapshot-reserve monitoring is required, not optional.](../../../diagrams/volume-006-enterprise-storage-data-protection/chapter-06-lvm-snapshot-exhaustion-flow.svg)
