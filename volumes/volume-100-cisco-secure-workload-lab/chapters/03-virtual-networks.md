...
**Objective.** Provide the "corporate" segment with internet egress via NAT on `192.168.170.0/24` (agents reach the cluster or SaaS over it).
...
**Objective.** Provide an isolated segment on `10.10.30.0/24` with **no host adapter** and **no DHCP**, so the only path to it is through `cw-gw`.
...
**Expected result.** No output. The host cannot reach `10.10.30.0/24` directly — the property that makes `cw-gw` the single, complete enforcement point for the PLC.
...
**Step 2.** The OT cell is reachable from the host only through `cw-gw`. You will not add a host route to it — testing the PLC in Chapter 08 is done from `cw-gw` or a Data Center host, which legitimately transit the enforcement point.
