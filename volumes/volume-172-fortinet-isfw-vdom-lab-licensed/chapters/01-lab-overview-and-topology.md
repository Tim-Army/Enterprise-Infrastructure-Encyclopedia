...
![Lab topology: a FortiGate Internal Segmentation Firewall segmenting a four-zone estate — web (zone APP), db (zone DB, :5432), hmi (zone MGMT), plc (zone OT, :502). Policies permit APP-to-DB PGSQL and MGMT-to-OT MODBUS; the implicit deny drops the MGMT-to-DB lateral flow. The OT tier is isolated in its own VDOM, crossed only by a scoped inter-VDOM link.](../../../diagrams/volume-172-fortinet-isfw-vdom-lab-licensed/chapter-01-lab-topology.svg)
