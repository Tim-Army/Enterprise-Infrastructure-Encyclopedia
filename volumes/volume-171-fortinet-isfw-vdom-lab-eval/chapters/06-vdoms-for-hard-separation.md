...
FGT (vdom) # edit OT
FGT (OT) # end
# move v2004 (plc) into the OT vdom
FGT # config global
...
Two VDOMs exist; `v2004` (the plc) now lives in `OT`, `v2001–v2003` (web/db/hmi) remain in `IT`.
...
The OT VDOM needs a matching policy on `itot1 → v2004` permitting MODBUS to plc.
